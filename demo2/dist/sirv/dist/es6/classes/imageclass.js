...
class _Image {
    ...
    addSrcset() {
        if (this._srcset) { // TODO because amount of tiles are different between 1x image and 1.5x image
...
class RImage extends EventEmitter {
    ...
    _convertImageSettings(imageSettings) {
        ...
        if (!imageSettings) { imageSettings = {}; }
        imageSettings = Object.assign({}, imageSettings);
        ...
        if (!result) {
            if (!(imageSettings.src.imageSettings && imageSettings.src.imageSettings.tile) && imageSettings.src.width) {
                const images = Object.entries(this.images)
                    .filter(v => v[1].image instanceof _TileImage === false)
                    .map(v => v[1]);
    ...
    _getBiggerImage(width, images, dontLoad) {
        ...
        return Object.entries(images)
            .map(value => value[1])
            .sort((a, b) => a.serverWidth - b.serverWidth)
            .find(value => width < value.serverWidth && (value.image.state === 2 || dontLoad)) || null;
    }
